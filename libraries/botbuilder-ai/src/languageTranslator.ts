...
export class LanguageTranslator implements Middleware {
    ...
    public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
        ...
        if (this.setUserLanguage != undefined) {
            const changedLanguage = await this.setUserLanguage(context);
            if (changedLanguage) {
        ...
        const targetLanguage = (this.nativeLanguages.indexOf(sourceLanguage) >= 0) ? sourceLanguage : this.nativeLanguages[0];
...
class MicrosoftTranslator implements Translator {
    public readonly TRANSLATEURL = 'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&includeAlignment=true&includeSentenceLength=true';
    public readonly DETECTURL = 'https://api.cognitive.microsofttranslator.com/detect?api-version=3.0';

    public apiKey: string;
    public postProcessor: PostProcessTranslator;
    ...
    public translateArrayAsync(options: TranslateArrayOptions): Promise<string> {
        ...
        const uri = `${this.TRANSLATEURL}&from=${options.from}&to=${options.to}`;
    ...
    public postProcessTranslation(response: string, orgTexts: string[]): TranslationResult[] {
        ...
        JSON.parse(response).map(responseElement => {
            const translationElement = responseElement.translations[0];
            if (translationElement.alignment != null) {
                const alignment = translationElement.alignment.proj as string;
...
export class PostProcessTranslator {
    ...
    public fixTranslation(sourceMessage: string, alignment: string, targetMessage: string): string {
        const numericMatches = sourceMessage.match(/[0-9]+/g);
        const containsNum = numericMatches != null;
        ...
        this.noTranslatePatterns.forEach(pattern => {
            ...
            const matches = sourceMessage.match(regExp);
            if (matches != null) {
        ...
        const alignments = alignment.trim().split(' ');

        const srcWords = this.splitSentence(sourceMessage, alignments);
        let trgWords = this.splitSentence(targetMessage, alignments, false);

        const alignMap = this.wordAlignmentParse(alignments, srcWords, trgWords);

        if (toBeReplaced.length > 0) {
            toBeReplaced.forEach(pattern => {
                const regExp = new RegExp(pattern, 'i');
                const match = regExp.exec(sourceMessage);

                const noTranslateStartChrIndex = match.index + match[0].indexOf(match[1]);
                const noTranslateMatchLength = match[1].length;
                let wrdIndx = 0;
                let chrIndx = 0;
                ...
                const wrdNoTranslate = srcWords.slice(srcIndx, srcIndx + newNoTranslateArrayLength);
        ...
        if (toBeReplacedByDictionary.length > 0) {
            toBeReplacedByDictionary.forEach(word => {
                ...
                const noTranslateStartChrIndex = match.index;

                const noTranslateMatchLength = match[0].length;

                let wrdIndx = 0;
                let chrIndx = 0;
                const newChrLengthFromMatch = 0;
                let srcIndx = -1;
                const newNoTranslateArrayLength = 1;
                ...
                const wrdNoTranslate = srcWords.slice(srcIndx, srcIndx + 1);
        ...
        if (containsNum) {
            numericMatches.forEach(numericMatch => {
                const srcIndx = srcWords.findIndex(wrd => wrd == numericMatch);
                trgWords = this.keepSrcWrdInTranslation(alignMap, srcWords, trgWords, srcIndx);
            });
        }
    ...
    private splitSentence(sentence: string, alignments: string[], isSrcSentence = true): string[] {
        ...
        const lastWrd = wrds[wrds.length - 1];
    ...
    private wordAlignmentParse(alignments: string[], srcWords: string[], trgWords: string[]): { [id: number] : number } {
        ...
        const sourceMessage = this.join(' ', srcWords);
        const trgMessage = this.join(' ', trgWords);

        alignments.forEach(alignData => {
            const wordIndexes = alignData.split('-');

            const srcStartIndex = parseInt(wordIndexes[0].split(':')[0]);
            const srcLength = parseInt(wordIndexes[0].split(':')[1]) - srcStartIndex + 1;
            const srcWrd = sourceMessage.substr(srcStartIndex, srcLength);
            const srcWrdIndex = srcWords.findIndex(wrd => wrd == srcWrd);

            const trgstartIndex = parseInt(wordIndexes[1].split(':')[0]);
            const trgLength = parseInt(wordIndexes[1].split(':')[1]) - trgstartIndex + 1;
            const trgWrd = trgMessage.substr(trgstartIndex, trgLength);
            const trgWrdIndex = trgWords.findIndex(wrd => wrd == trgWrd);
