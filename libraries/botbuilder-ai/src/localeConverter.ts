...
export class LocaleConverter implements Middleware {
    ...
    public async onTurn(context: TurnContext, next: () => Promise<void>): Promise<void> {
        ...
        if (this.setUserLocale != undefined) {
            const changedLocale = await this.setUserLocale(context);
            if (changedLocale) {
...
class MicrosoftLocaleConverter implements ILocaleConverter {
    ...
    public mapLocaleToFunction: { [id: string] : DateAndTimeLocaleFormat } = {};
    ...
    public convert(message: string, fromLocale: string, toLocale: string): Promise<string> {
        ...
        try {
            ...
            dates.forEach(date => {
                if (date.range) {
                    if (date.type == 'time') {
                        const convertedStartDate = this.formatTime(date.dateTimeObj, toLocale);
                        const convertedEndDate = this.formatTime(date.endDateTimeObj, toLocale);
                        processedMessage = processedMessage.replace(date.text, `${convertedStartDate} - ${convertedEndDate}`);
                    } else if (date.type == 'date') {
                        const convertedStartDate = this.formatDate(date.dateTimeObj, toLocale);
                        const convertedEndDate = this.formatDate(date.endDateTimeObj, toLocale);
                        processedMessage = processedMessage.replace(date.text, `${convertedStartDate} - ${convertedEndDate}`);
                    } else {
                        const convertedStartDate = this.formatDateAndTime(date.dateTimeObj, toLocale);
                        const convertedEndDate = this.formatDateAndTime(date.endDateTimeObj, toLocale);
                        processedMessage = processedMessage.replace(date.text, `${convertedStartDate} - ${convertedEndDate}`);
                    }
                } else {
                    if (date.type == 'time') {
                        const convertedDate = this.formatTime(date.dateTimeObj, toLocale);
                        processedMessage = processedMessage.replace(date.text, convertedDate);
                    } else if (date.type == 'date') {
                        const convertedDate = this.formatDate(date.dateTimeObj, toLocale);
                        processedMessage = processedMessage.replace(date.text, convertedDate);
                    } else {
                        const convertedDateTime = this.formatDateAndTime(date.dateTimeObj, toLocale);
    ...
    private extractDates(message: string, fromLocale: string): TextAndDateTime[] {
        ...
        const model = new DateTimeRecognizers.DateTimeRecognizer(culture).getDateTimeModel();
        const results = model.parse(message);
        ...
        results.forEach(result => {
            ...
            const resolutionValues = result.resolution.values[0];
            const type = result.typeName.replace('datetimeV2.', '');
