    ...
    public read(keys: string[]): Promise<StoreItems> {
        ...
        return this.ensureCollectionExists().then((collectionLink) => {
            return new Promise<StoreItems>((resolve, reject) => {
                ...
                const query = this.client.queryDocuments(collectionLink, querySpec);
                const getNext = function (query) {
        ...
        return this.ensureCollectionExists().then(() => {
            return Promise.all(Object.keys(changes).map(k => {
                ...
                return new Promise((resolve, reject) => {
                    const handleCallback = (err, data) => err ? reject(err) : resolve();

                    const eTag = changes[k].eTag;
                    if (!eTag || eTag === '*') {
                        // if new item or * then insert or replace unconditionaly
                        const uri = UriFactory.createDocumentCollectionUri(this.settings.databaseId, this.settings.collectionId);
                        this.client.upsertDocument(uri, documentChange, { disableAutomaticIdGeneration: true }, handleCallback);
                    } else if (eTag.length > 0) {
                        // if we have an etag, do opt. concurrency replace
                        const uri = UriFactory.createDocumentUri(this.settings.databaseId, this.settings.collectionId, documentChange.id);
