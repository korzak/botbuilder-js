...
export function encryptString(value: string, secret: string, iv?: string): string {
    ...
    const secretKey = crypto.createHash('sha256').update(secret, 'utf8').digest();
    const ivKey = Buffer.from(((iv || '').toLowerCase() + '                ').substr(0, 16), 'utf8');
    const cipher = crypto.createCipheriv('aes256', secretKey, ivKey);
...
export function decryptString(encryptedValue: string, secret: string, iv?: string): string {
    ...
    const secretKey = crypto.createHash('sha256').update(secret || '', 'utf8').digest();
    const ivKey = Buffer.from(((iv || '').toLowerCase() + '                ').substr(0, 16), 'utf8');
    const decipher = crypto.createDecipheriv('aes256', secretKey, ivKey);
