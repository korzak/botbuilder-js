...
export class JwtTokenExtractor {
    ...
    public async getIdentityFromAuthHeader(authorizationHeader: string, channelId: string): Promise<ClaimsIdentity | null> {
        ...
        if (parts.length == 2) {
            return this.getIdentity(parts[0], parts[1], channelId);
        }
    ...
    private async validateToken(jwtToken: string, channelId: string): Promise<ClaimsIdentity> {
        ...
        try {
            const decodedPayload = <any>jwt.verify(jwtToken, metadata.key, this.tokenValidationParameters);
            ...
            const claims: Claim[] = Object.keys(decodedPayload).reduce(function (acc, key) {
